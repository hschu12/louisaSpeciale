# Constr    (rand)  cut   card
# Flip      (rand)  cap   tabu
# Step    first   (prob)  sim


# Read file
comb = read.csv('comb-results.csv')

# Change factor into numerics
comb$best.max.flow = as.numeric(levels(comb$best.max.flow))[comb$best.max.flow]
comb$initial.max.flow = as.numeric(levels(comb$initial.max.flow))[comb$initial.max.flow]


# Remove empty rows
NA_list = list()
for (i in 1:nrow(comb)) {
  if (is.na(comb$best.max.flow[i])) {
    NA_list = c(NA_list, i)
  }   
}
removed = 0
